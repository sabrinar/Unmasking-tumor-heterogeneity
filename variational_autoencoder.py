...
dict=scipy.io.loadmat('AllCells.mat')#GSE89567, cml_selected
x_t=dict['AllCells']
#x_t=dict['syn_expr'];
...
x_train=np.concatenate((x_train,x_train[2091:2095,:]),axis=0)# forcml
...
class CustomVariationalLayer(Layer):
    ...
    def vae_loss(self, x, x_decoded_mean):
        ...
        return K.mean(xent_loss/1000 + kl_loss/1000)
...
for n_components in n_components_range:
    ...
    gmm.fit(hhj)
    bic.append(gmm.aic(hhj))

bic = np.array(bic)+1*np.log(size[0])*n_components_range*latent_dim
...
for i in range(0,labels.max()+1):
    ax3D.scatter(hhj[labels==i, 0], hhj[labels==i, 1], hhj[labels==i, 2],alpha=.5, color=color_iter[i])
...
scipy.io.savemat('np_vector.mat', {'vect':hhj1})
